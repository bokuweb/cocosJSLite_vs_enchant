...
window.Game = (function() {
  ...
  function Game(w, h) {
    ...
    enchant();
    this._game = new Core(w, h);
    this._game.fps = 60;
  }

  Game.prototype.start = function(_num) {
    this._num = _num;
    this._game.preload(["res/test1.png"]);
    return this._load().then(this._play);
  };
  ...
  Game.prototype._play = function() {
    ...
    for (i = j = 0, ref = this._num; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
      ...
      this._game.currentScene.addChild(sprite);
    }
    fps = new Label();
    fps.color = '#fff';
    this._game.currentScene.addChild(fps);
    return fps.addEventListener('enterframe', function() {
      var currentTime;
      currentTime = new Date();
      fps = 1000 / (currentTime - this._oldTime);
      this._oldTime = currentTime;
      return this.text = fps.toFixed(2) + " FPS";
    });
  };
...
},{"./enchant":2,"jquery":3}],2:[function(require,module,exports){
/**
 * enchant.js v0.8.2
 * http://enchantjs.com
 *
